...
def test_missing_baseline_fails_before_discord_read_or_cursor_change(
    tmp_path, monkeypatch
):
    args, _before = execute_fixture(tmp_path)
    install_selected_state(args, "1540000000000000001")
    state_before = Path(args.state).read_bytes()
    queue_before = Path(args.queue).read_bytes()
    ...
    assert Path(args.state).read_bytes() == state_before
    assert Path(args.queue).read_bytes() == queue_before
