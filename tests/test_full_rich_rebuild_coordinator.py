...
TEST_INTEGRITY_ENTRY_COUNT = 182
...
def make_integrity_bound_runtime(tmp_path: Path, monkeypatch):
    ...
    entries = full.validate_expected_entries(
        entry_values(TEST_INTEGRITY_ENTRY_COUNT),
        expected_count=TEST_INTEGRITY_ENTRY_COUNT,
    )
    ...
    entries_payload = {
        ...
        "entryCount": TEST_INTEGRITY_ENTRY_COUNT,
    ...
    config_payload = {
        ...
        "expectedEntryCount": TEST_INTEGRITY_ENTRY_COUNT,
    ...
    daily_runner_path = scripts / "run_daily_sync_v3.py"
    daily_runner_path.write_text("# integrity-bound daily runner fixture\n", encoding="utf-8")
    daily_runner_path.chmod(0o600)
    managed_component_path = scripts / "run_managed_component.py"
    managed_component_path.write_text(
        "# integrity-bound managed component fixture\n", encoding="utf-8"
    )
    managed_component_path.chmod(0o600)

    component_paths = {
        "adapter": adapter_path,
        "richCore": rich_core_path,
        "coordinator": coordinator_path,
        "dailyRunner": daily_runner_path,
        "managedComponent": managed_component_path,
    ...
    manifest_path = skill_root / full.RUNTIME_MANIFEST_RELATIVE_PATH
...
def test_entry_count_is_bound_by_approved_baseline_not_product_constant(tmp_path):
    config = make_config(tmp_path, count=177)
    validated = full.validate_config(config)
    assert validated.approved_baseline.expected_entry_count == 177
    assert len(validated.approved_baseline.expected_entries) == 177
...
def test_integrity_bundle_binds_all_components_and_rechecks_under_lock(
    ...
    assert baseline.expected_entry_count == TEST_INTEGRITY_ENTRY_COUNT
