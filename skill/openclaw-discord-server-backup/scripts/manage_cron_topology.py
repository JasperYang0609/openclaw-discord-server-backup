...
def validate_manifest(manifest: dict[str, Any]) -> None:
    ...
    if any(
        ...
        raise CronManagerError(
            "all daily-sync jobs must use the deterministic runner and shared ownership contract"
        )
...
def render_jobs(manifest: dict[str, Any], context: RenderContext) -> list[dict[str, Any]]:
    ...
    for row in manifest["jobs"]:
        ...
        desired.append(job)
    daily_jobs = [
        job for job in desired if str(job.get("role") or "").startswith("daily-sync-")
    ]
    if any(
        job.get("sessionTarget") != "isolated"
        or (job.get("payload") or {}).get("kind") != "command"
...
def desired_daily_lock_path(desired: list[dict[str, Any]], workspace: Path) -> Path:
    """Resolve the one reviewed shared lock used by all deterministic daily slots."""
...
class OpenClawCronClient:
    ...
    def daily_lock_canary(self, workspace: Path, lock_path: Path) -> None:
        ...
        workspace_child(workspace, str(lock_path), label="daily lock path")
        with tempfile.TemporaryDirectory(prefix="openclaw-daily-lock-canary-") as temp:
            root = Path(temp)
            canary_lock = root / "shared.lock"
            ...
            first = subprocess.Popen(
                command_a, text=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE
            )
            try:
                ...
                second = subprocess.run(
                    command_b, text=True, capture_output=True, timeout=30, check=False
                )
