        ...
        with slot_manager as slot:
            ...
            with slot.begin_incremental(begin_request) as session:
                if type(session) is not adapter_module.IncrementalSessionV3:
                    raise DailySyncError("rich_core_contract_unsupported")
                # Daily sync is incremental-only. Prove every selected entry
                # has a valid rich CURRENT before resolving credentials or
                # making the first Discord request. A partial bootstrap would
                # otherwise let early entries advance while a later entry has
                # no baseline.
                for rich_binding in rich_bindings:
                    try:
